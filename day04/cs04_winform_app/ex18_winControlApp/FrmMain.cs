...
namespace ex18_winControlApp
{
    ...
    {
        Random rand = new Random(); // 트리뷰 로드이름으로 사용할 랜덤값
        public FrmMain()
        {
        ...
        void ChangeFont()
        {
            if (CboFonts.SelectedIndex < 0) // 아무것도 선택 X
                return;

            FontStyle style = FontStyle.Regular; // 일반 글자체로 초기화

            if (ChkBold.Checked) // 볼드체 체크박스 체크하면
                style |= FontStyle.Bold;

            if (ChkItalic.Checked) // 이탤릭체 체크박스 체크하면
        ...
        private void BtnModal_Click(object sender, EventArgs e)
        {
            ...
            FrmModal.ShowDialog(); // 모달창 띄우기, 부모창 선택불가
        ...
        private void BtnModaless_Click(object sender, EventArgs e)
        {
            ...
            // 모달리스창을 부모 정중앙에 위치할대는 아래와 같이 작업해야함
            ...
            FrmModaless.Show(); // 모달리스창 띄우기, 부모창 선택가능
        }

        private void BtnMsgBox_Click(object sender, EventArgs e)
        {
            MessageBox.Show(TxtSampleText.Text, "메시지박스", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
        ...
        private void BtnLoad_Click(object sender, EventArgs e)
        {
            ...
            {
                //MessageBox.Show(DlgOpenImage.FileName);
        ...
        private void BtnFileLoad_Click(object sender, EventArgs e)
        {
            ...
            dialog.Multiselect = false; // 여러개 파일 선택금지
            dialog.Filter = "Text Files(*.txt;*.cs;*.py) | *.txt;*.cs;*.py";
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                // UTF-8 한글이 깨짐. EUC-KR(Window 949), UTF-8(BOM)은 한글 문제 없음
        ...
        private void BtnFileSave_Click(object sender, EventArgs e)
        {
            ...
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                RtxEditor.SaveFile(dialog.FileName, RichTextBoxStreamType.PlainText);
            }
        }
        ...
        private void DoRealWork(BackgroundWorker worker, DoWorkEventArgs e)
        {
            ...
            for (var i = 0; i <= maxValue ; i++) 
            {
                if (worker.CancellationPending) // 중간 취소 여부
                {
                    e.Cancel = true;
                    break;
                }
                else
                {
                    ...
                    // ProgressChangeEventArgs내의 ProgressPercentage 속성에 값이 들어감.
