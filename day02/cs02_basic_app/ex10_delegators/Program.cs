...
namespace ex10_delegators
{
    ...
    class EventListener // 이벤트가 발생하는 지 듣고있는 객체
    {
        ...
        {
            Console.WriteLine($"{name}, 무슨일이 발생했다 : {message}");
        }
        ...
        internal class Program
        {
            static void Main(string[] args)
            {
                ...
                // 일반적인 함수 호출에서 한번에 어러개의 함수 실행이 가능하냐? 불가능
                ...
                notifier.EventOccured ("메일을 받음");  // 1, 2, 3 실행
                Console.WriteLine();

                notifier.EventOccured -= listener2.SomethingHappened; // 리스너2번의 함수는 실행 X
                ...
                Callback = new MyDelegate(cal.Plus); // int a, int b가 아닌 Calculator객체의 plus() 메서드를 전달
