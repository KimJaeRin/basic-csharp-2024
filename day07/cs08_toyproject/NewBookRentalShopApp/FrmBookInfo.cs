...
namespace NewBookRentalShopApp
{
    public partial class FrmBookInfo : MetroForm
    {
        ...
        {
            isNew = true;
            TxtAuthor.Text = TxtNames.Text = string.Empty;
            TxtAuthor.Focus(); // 순번은 자동증가하기때문에 입력 불가
            CboDivision.SelectedIndex = -1;
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(helper.common.ConnString))
                {
                    ...
                    SqlParameter prmReleaseDate = new SqlParameter("@ReleaseDate", DtReleaseDate.Value);
            ...
            TxtBookIdx.Text = TxtAuthor.Text = string.Empty; // 모든 입력값 삭제
            CboDivision.SelectedIndex = -1;
            TxtNames.Text = TxtIsbn.Text = string.Empty;
            DtReleaseDate.Value = DateTime.Now;
            NudPrice.Value = 0;
        ...
        private void DgvResult_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1) // 아무것도 선택하지 않으면 -1
            {
                ...
                DtReleaseDate.Value = DateTime.Parse(selData.Cells[5].Value.ToString()); // 출판일
                TxtIsbn.Text = selData.Cells[6].Value.ToString(); // ISBN
                NudPrice.Value = Decimal.Parse(selData.Cells[7].Value.ToString()); // 책 가격
